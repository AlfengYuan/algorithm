...
class encode_file_persistence : virtual public mqtt::iclient_persistence {
...
public:
    ...
    encode_file_persistence(const string &encodeKey) : encodeKey_(encodeKey) {}
    ...
    void open(const string &clientID, const string &serverURI) override {
        if (clientID.empty() || serverURI.empty()) throw mqtt::persistence_exception();
    ...
    // Returns the keys in this persistence data store.
    ...
    // it order - and a scatter/gather like writev() would be fine. But...
    ...
    void put(const string &key, const std::vector<mqtt::string_view> &bufs) override {
        ...
        if (!os) throw mqtt::persistence_exception();
    ...
    string get(const string &key) const override {
        ...
        if (!is) throw mqtt::persistence_exception();
...
int main(int argc, char *argv[]) {
...
#else
    encode_file_persistence persistence("elephant");
    mqtt::async_client cli(address, CLIENT_ID, MAX_BUFFERED_MSGS, &persistence);
#endif
    ...
    try {
        ...
        // The time at which to read the next sample, starting now
